 ...
 */
export function resolveGooseBinary() {
    ...
    if (!pkg) {
        throw new Error(`No goose binary available for ${key}. Set GOOSE_BINARY to the path of a goose binary.`);
    }
    try {
        ...
        throw new Error(`goose binary package ${pkg} is not installed. Set GOOSE_BINARY or install the native package.`);
    }
}
